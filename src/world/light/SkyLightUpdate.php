...
class SkyLightUpdate extends LightUpdate{
	...
	public function recalculateNode(int $x, int $y, int $z) : void{
		...
		$lightFilter = BlockFactory::getInstance()->lightFilter;
		$diffusesSkyLight = BlockFactory::getInstance()->diffusesSkyLight;

		if($yPlusOne === $oldHeightMap){ //Block changed directly beneath the heightmap. Check if a block was removed or changed to a different light-filter.
			$newHeightMap = self::calculateHeightMap($x & 0x0f, $z & 0x0f, $chunk, $lightFilter, $diffusesSkyLight);
		...
		if($newHeightMap > $oldHeightMap){ //Heightmap increase, block placed, remove sky light
			...
			$this->setAndUpdateLight($x, $y, $z, max(0, $this->getHighestAdjacentLight($x, $y, $z) - $lightFilter[$source->getFullId()]));
		}
	}
	...
	public function recalculateChunk(int $chunkX, int $chunkZ) : void{
		...
		for($x = 0; $x < 16; ++$x){
			for($z = 0; $z < 16; ++$z){
				$heightMap = self::calculateHeightMap($x, $z, $chunk, $lightFilter, $diffusesSkyLight);
				...
				if($heightMap > 0){
					$this->setAndUpdateLight(
						...
						$heightMap - 1,
						($chunkZ << 4) | $z,
						15 - $lightFilter[$chunk->getFullBlock($x, $heightMap - 1, $z)]
					);
				}
			}
		}
	 ...
	 *
	 * @param \SplFixedArray|int[] $lightFilter
	 * @param \SplFixedArray|bool[] $diffusesSkyLight
	 * @phpstan-param \SplFixedArray<int> $lightFilter
	 * @phpstan-param \SplFixedArray<bool> $diffusesSkyLight
	 *
	 * @return int New calculated heightmap value (0-256 inclusive)
	 */
	private static function calculateHeightMap(int $x, int $z, Chunk $chunk, \SplFixedArray $lightFilter, \SplFixedArray $diffusesSkyLight) : int{
		$max = $chunk->getHighestBlockAt($x, $z);
		$y = $max;
		for(; $y >= 0; --$y){
			if($lightFilter[$state = $chunk->getFullBlock($x, $y, $z)] > 1 or $diffusesSkyLight[$state]){
				break;
			}
		}
